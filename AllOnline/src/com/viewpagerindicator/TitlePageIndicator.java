...
/**
 * A TitlePageIndicator is a PageIndicator which displays the title of left rootView
 * (if exist), the title of the current select rootView (centered) and the title of
 * the right rootView (if exist). When the user scrolls the ViewPager then titles
 * are also scrolled.
 */
public class TitlePageIndicator extends View implements PageIndicator
{
	...
	/** Left and right side padding for not active rootView titles. */
	...
	public TitlePageIndicator(Context context, AttributeSet attrs, int defStyle)
	{
		...
		// Retrieve the colors to be used for this rootView and apply them.
	 ...
	 * (non-Javadoc)
	 *
	 * @see android.rootView.View#onDraw(android.graphics.Canvas)
	 */
	@Override
	protected void onDraw(Canvas canvas)
	{
		...
		// Verify if the current rootView must be clipped to the screen
		...
		if (mCurrentPage > 0)
		{
			for (int i = mCurrentPage - 1; i >= 0; i--)
			{
				...
				if (bound.left < leftClip)
				{
					...
					// Except if there's an intersection with the right rootView
		...
		if (mCurrentPage < countMinusOne)
		{
			for (int i = mCurrentPage + 1; i < count; i++)
			{
				...
				if (bound.right > rightClip)
				{
					...
					// Except if there's an intersection with the left rootView
	...
	/**
	 * Calculate the bounds for a rootView's title
	 *
	 * @param index
	 * @param paint
	 * @return
