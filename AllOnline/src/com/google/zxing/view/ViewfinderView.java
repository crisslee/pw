...
/**
 * This rootView is overlaid on top of the camera preview. It adds the viewfinder rectangle and partial
...
public final class ViewfinderView extends View {
  ...
  public ViewfinderView(Context context, AttributeSet attrs) {
    ...
    labelText = array.getString(R.styleable.ViewfinderView_label_text);
      labelTextSize = array.getFloat(R.styleable.ViewfinderView_label_text_size, 39f);
  ...
  public void onDraw(Canvas canvas) {
    ...
    private void drawTextInfo(Canvas canvas, Rect frame) {
        //文字背景
        int margin = DisplayUtils.dp2px(getContext(), 25.0f);
        int baseX = frame.left + frame.width() / 2;
        int baseY = frame.top - margin;
        int textBgColor = Color.parseColor("#000000");
        paint.setColor(textBgColor);
        Rect textR = new Rect();
        paint.getTextBounds(labelText, 0, labelText.length(), textR);
        int textW = textR.right - textR.left;
        int textH = textR.bottom - textR.top;
        int pT = DisplayUtils.dp2px(getContext(), 5);
        int pL = DisplayUtils.dp2px(getContext(), 10);
        RectF rf = new RectF(baseX - textW / 2 - pL, baseY - textH - pT, baseX + textW / 2 + pL, baseY + pT);
        canvas.drawRoundRect(rf, 40, 40, paint);
        //文字
        paint.setColor(labelTextColor);
        paint.setTextSize(labelTextSize);
        paint.setTextAlign(Paint.Align.CENTER);
        int ty = (int) ((rf.top + rf.bottom - textR.top - textR.bottom) / 2);
        canvas.drawText(labelText, rf.centerX(), ty, paint);
    }
...
//    canvas.drawRect(frame.left + 2, middle - 1, frame.right - 1, middle + 2, paint);
      ...
      int startColor = Color.parseColor("#9900D5FF");
      int endColor = Color.parseColor("#0000D5FF");
      LinearGradient l = new LinearGradient(frame.left, scannerStart - 300, frame.left, scannerStart,
          endColor, startColor, Shader.TileMode.CLAMP);
      paint.setShader(l);
      int top = scannerStart - 300 < frame.top ? frame.top : scannerStart - 300;
      RectF f = new RectF(frame.left, top, frame.right, scannerStart);
