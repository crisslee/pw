...
import com.goomeim.adapter.EmojiconGridAdapter;
import com.goomeim.adapter.EmojiconPagerAdapter;
...
public class GMEmojiconPagerView extends ViewPager
{

    private Context context;
    private List<GMEmojiconGroupEntity> groupEntities;
    private List<GMEmojicon> totalEmojiconList = new ArrayList<GMEmojicon>();
    ...
    private GMEmojiconPagerViewListener pagerViewListener;
    private List<View> viewpages;

    public GMEmojiconPagerView(Context context, AttributeSet attrs)
    {
        super(context, attrs);
        this.context = context;
    }

    public GMEmojiconPagerView(Context context)
    {
        this(context, null);
    }

    public void init(List<GMEmojiconGroupEntity> emojiconGroupList, int emijiconColumns, int bigEmojiconColumns)
    {
        ...
        {
            GMEmojiconGroupEntity group = groupEntities.get(i);
            List<GMEmojicon> groupEmojicons = group.getEmojiconList();
    ...
    public void setPagerViewListener(GMEmojiconPagerViewListener pagerViewListener)
    {
     ...
     * @return
     */
    public List<View> getGroupGridViews(GMEmojiconGroupEntity groupEntity)
    {
        List<GMEmojicon> emojiconList = groupEntity.getEmojiconList();
        int itemSize = emojiconColumns * emojiconRows - 1;
        int totalSize = emojiconList.size();
        GMEmojicon.Type emojiType = groupEntity.getType();
        if (emojiType == GMEmojicon.Type.BIG_EXPRESSION)
        {
        ...
        for (int i = 0; i < pageSize; i++)
        {
            ...
            else
            {
                ...
                int gvHeight = itemWidth * 3 + spacing * 4 + spacing * 2;
                gv.setPadding(spacing, spacing, spacing, spacing);
                gv.setHorizontalSpacing(spacing);
                gv.setVerticalSpacing(spacing);
                LinearLayout.LayoutParams params = (android.widget.LinearLayout.LayoutParams) gv.getLayoutParams();
                params.width = AllOnlineApp.screenWidth;
                params.height = gvHeight;
                gv.setLayoutParams(params);
                getLayoutParams().height = gvHeight;
            }
            List<GMEmojicon> list = new ArrayList<GMEmojicon>();
            if (i != pageSize - 1)
            {
            ...
            if (emojiType != GMEmojicon.Type.BIG_EXPRESSION)
            {
                GMEmojicon deleteIcon = new GMEmojicon();
                deleteIcon.setEmojiText(GMSmileUtils.DELETE_KEY);
                list.add(deleteIcon);
            }

            final EmojiconGridAdapter gridAdapter = new EmojiconGridAdapter(context, 1, list, emojiType, itemWidth);
            ...
            gv.setOnItemClickListener(new OnItemClickListener()
            {
                ...
                {
                    GMEmojicon emojicon = gridAdapter.getItem(position);
                    if (pagerViewListener != null)
                    {
                        String emojiText = emojicon.getEmojiText();
                        if (emojiText != null && emojiText.equals(GMSmileUtils.DELETE_KEY))
                        {
     ...
     * @param groupEntity
     */
    public void addEmojiconGroup(GMEmojiconGroupEntity groupEntity, boolean notifyDataChange)
    {
     ...
     * @return
     */
    private int getPageSize(GMEmojiconGroupEntity groupEntity)
    {
        List<GMEmojicon> emojiconList = groupEntity.getEmojiconList();
        int itemSize = emojiconColumns * emojiconRows - 1;
        int totalSize = emojiconList.size();
    ...
    public interface GMEmojiconPagerViewListener
    {
        /**
