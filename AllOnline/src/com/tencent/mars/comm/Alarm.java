...
public class Alarm extends BroadcastReceiver {
    ...
    public static boolean start(final long id, final int after, final Context context) {
        ...
        if (0 > after) {
            TLog.e(TAG, "id:%d, after:%d", id, after);
            return false;
        }

        if (null == context) {
            TLog.e(TAG, "null==context, id:%d, after:%d", id, after);
            return false;
        }
        ...
        synchronized (alarm_waiting_set) {
            if (null == wakerlock) {
                ...
                TLog.i(TAG, "start new wakerlock");
            }
    ...
    public void onReceive(final Context context, Intent intent) {
        ...
        if (pid != Process.myPid()) {
            TLog.w(TAG, "onReceive id:%d, pid:%d, mypid:%d", id, pid, Process.myPid());
            return;
        }
        ...
        synchronized (alarm_waiting_set) {
            ...
            while (it.hasNext()) {
                ...
                TLog.i(TAG, "onReceive id=%d, curId=%d", id, curId);
                if (curId.equals(id)) {

                    TLog.i(TAG, "onReceive find alarm id:%d, pid:%d, delta miss time:%d", id, pid, SystemClock.elapsedRealtime() - (Long) next[TSetData.WAITTIME.ordinal()]);
                    it.remove();
                    found = true;
                    break;
                }
            ...
            if (!found)
                TLog.e(TAG, "onReceive not found id:%d, pid:%d, alarm_waiting_set.size:%d", id, pid, alarm_waiting_set.size());
        }
