...
public class CropperImageActivity extends BaseActivity
{
	...
	protected Handler mHandler = new Handler()
	{
		...
		public void handleMessage(Message msg)
		{
			if (msg.what == 0x111)
			{
				...
				/*
				 * if(mCameraSdkParameterInfo.isFilter_image()){ //跳转到滤镜
				 * Constants.bitmap=itbmp;
				 * 
				 * Bundle b=new Bundle();
				 * b.putSerializable(CameraSdkParameterInfo.EXTRA_PARAMETER,
				 * mCameraSdkParameterInfo); Intent intent = new
				 * Intent(mContext, FilterImageActivity.class);
				 * intent.putExtras(b); startActivity(intent); } else{ //生成图片返回
				 * String
				 * path=ImageUtils.saveAsBitmap(CropperImageActivity.this,
				 * itbmp, Constants.folderName, null); itbmp.recycle();
				 * PhotoPickActivity.instance.getForResultComplate(path); }
				 */
	...
	private void show()
	{
		if (flag == 0)
		{
			...
			// Toast.makeText(Activity_Cropper.this,
			// "屏幕已锁定,请按全屏键解锁",Toast.LENGTH_SHORT).show();
			// 再次点击 恢复之前可以放缩状态
		}
		else if (flag == 1)
		{
